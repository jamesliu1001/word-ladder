...
class LadderGraph(Graph):
  def __init__(self, word_list:list[str]):
    #self.test = ""
    self.__adj_list = self.build_adj_list(word_list)
    super().__init__(self.__adj_list)
  ...
  def build_adj_list(self, word_list: list[str])-> dict[str, set[str]]:
    ...
    for key in word_list:
      ...
      for word1 in word_list: 
        if self.hamming_distance(key, word1) == 1: 
           adjList[key].add(word1.lower())
  ...
  def is_valid_word(self, word:str) -> bool:
    ...
    if word.lower() in self.__adj_list: 
       return True
  ...
  def is_valid_ladder(self, ladder:tuple[str]) -> bool:
    ...
    if ladder[-1].lower() == ladder[0].lower():
       return False

    path = list(ladder)

    for index in range (len(path)-1): 
      if not self.is_vertex(path[index].lower()) or not self.is_vertex(path[index+1].lower()):
        return False
      if path[index+1].lower() in self.__adj_list[path[index].lower()]:
        continue
      else: 
        return False
    return True
  ...
  def get_shortest_ladder(self, start:str, target:str) -> tuple[str]:
    ...
    if self.is_valid_word(start) == False or self.is_valid_word(target) == False:
       return tuple()
  ...
  def get_all_ladders(self, start:str, target:str, max_rungs=math.inf) -> set[tuple[str]]:
    ''' Finds all the paths connecting the start vertex with the target vertex.
...
if __name__ == "__main__":
  ...
  myLadderGraph = LadderGraph(get_valid_word_list())
  # word_list = []


  # # print(myLadderGraph.build_adj_list(small_dictionary))
  ...
  print(len(myLadderGraph.adj_list))